...
- Tải bài học offline (Offline Learning Cache):
  ...
  - Thêm nút "Tải offline" (Premium required) tải đệ quy toàn bộ chương và bài học. Khi bấm tải offline nhưng chưa phải Premium, hộp thoại cảnh báo sẽ dẫn người dùng trực tiếp đến trang đăng ký VIP [subscription_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/subscription_screen.dart) thay vì cửa hàng chung.
- Luyện viết TOPIK II (Câu 51-54) & Chấm điểm động:
  - Thêm endpoint `POST /ai/generate-writing-question` ở backend NestJS để sinh đề bài thi viết ngẫu nhiên theo bối cảnh thực tế.
  - Hỗ trợ chấm bài viết chấm điểm nghiêm ngặt theo thang điểm barem tương ứng (10đ cho Câu 51/52, 30đ cho Câu 53, 50đ cho Câu 54).
  - Refactor [ai_writing_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/ai_writing_screen.dart) sang giao diện 2 tab (Viết tự do & Luyện thi TOPIK II), tích hợp bảng bí quyết làm bài, xem đáp án mẫu collapsible kèm giải nghĩa ngữ pháp, và vòng tròn chấm điểm động.
  - Đồng bộ hiển thị điểm số tối đa động (ví dụ: `8/10`, `22/30`, `38/50`) trên màn hình lịch sử [writing_history_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/writing_history_screen.dart) và chi tiết [writing_detail_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/writing_detail_screen.dart) dựa trên tiền tố `"Câu X: "` trong đề bài.
  - Sửa và tối ưu hóa cảnh báo phân tích Flutter trong `ai_writing_screen.dart` để code đạt trạng thái 100% warning-free.
- Nâng cấp Luyện viết TOPIK II Câu 51/52 và Cấu hình Học Offline:
  - Điều chỉnh giao diện [ai_writing_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/ai_writing_screen.dart) cho Câu 51 và Câu 52: Tách thành 2 ô nhập liệu riêng biệt cho ㉠ và ㉡ thay vì bắt học viên chép lại cả văn bản, tự động nối chuỗi đáp án khi gửi chấm bài.
  - Cập nhật backend `getAdjustedSystemPrompt` trong [ai.service.ts](file:///g:/NODEJS/koreanlearningapp/apps/api/src/modules/ai/ai.service.ts) để hướng dẫn AI chấm điểm nghiêm ngặt các câu trả lời điền khuyết và trả về văn bản hoàn chỉnh trong bài sửa.
  - Tích hợp dependency `flutter_markdown` và chuyển đổi hiển thị đáp án mẫu, giải nghĩa ngữ pháp, nhận xét của giám khảo AI, và văn bản sửa lỗi thành widget `MarkdownBody` trực quan, đẹp mắt.
  - Cập nhật [auth_provider.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/providers/auth_provider.dart): Khi khởi động app offline, nạp profile người dùng đã lưu cache cục bộ (`cached_user_profile` trong SharedPreferences) thay vì xóa token và bắt đăng nhập lại. Tự động lưu cache profile mỗi khi đăng nhập/đăng ký/làm mới trực tuyến thành công.
  - Thêm Hive caching cho danh sách khóa học, đề thi TOPIK và daily goal trong [home_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/home_screen.dart) và danh sách đề thi trong [topik_exams_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/topik_exams_screen.dart) khi ngoại tuyến.
- Lưu trữ và hiển thị Đáp án mẫu & Giải nghĩa ngữ pháp trong Lịch sử luyện viết:
  - Thêm cột `sampleAnswer` và `explanation` (loại String?) vào model `AIWritingPractice` trong [schema.prisma](file:///g:/NODEJS/koreanlearningapp/apps/api/prisma/schema.prisma) và đồng bộ cơ sở dữ liệu qua Prisma client.
  - Cập nhật NestJS controller DTO và service trong [ai.controller.ts](file:///g:/NODEJS/koreanlearningapp/apps/api/src/modules/ai/ai.controller.ts) và [ai.service.ts](file:///g:/NODEJS/koreanlearningapp/apps/api/src/modules/ai/ai.service.ts) để lưu lại dữ liệu này.
  - Cập nhật mobile app [api_client.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/core/api_client.dart) và [ai_writing_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/ai_writing_screen.dart) gửi kèm các trường trên khi chấm điểm.
  - Thiết lập hiển thị trực quan các thẻ Đáp án mẫu và Giải nghĩa bằng `MarkdownBody` trong màn hình chi tiết lịch sử [writing_detail_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/writing_detail_screen.dart).
  - Tự động hóa sinh Đáp án mẫu & Giải thích trong lúc chấm bài viết TOPIK (51-54): Cập nhật system prompt của `correctWriting` trong [ai.service.ts](file:///g:/NODEJS/koreanlearningapp/apps/api/src/modules/ai/ai.service.ts) để AI luôn sinh đáp án mẫu và giải thích nếu chưa có, sau đó tự động lưu vào DB.
  - Cập nhật UI màn hình luyện viết [ai_writing_screen.dart](file:///g:/NODEJS/koreanlearningapp/apps/mobile/lib/screens/ai_writing_screen.dart) hiển thị các thẻ Đáp án mẫu và Giải nghĩa bằng `MarkdownBody` ngay dưới kết quả chấm điểm trực quan.
  - Thêm hiệu ứng cuộn tự động (Auto-scroll) & Thông báo (SnackBar) khi chấm xong: Khi chấm điểm thành công, hiển thị thông báo đã chấm điểm xong và tự động cuộn xuống phần kết quả phân tích của giám khảo AI.
  - Di chuyển nút "Xem đáp án mẫu & Giải thích đề bài": Chuyển nút bật/tắt hiển thị đáp án mẫu và giải thích của đề bài xuống dưới cùng của giao diện Tab luyện viết TOPIK.